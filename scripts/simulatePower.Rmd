...
* `r sprintf("%.03g%%", power %>% filter(term == "MBSR1") %>% pull(power) * 100)` 
  power to detect an effect size of 
  `r sprintf("%.03g", power %>% filter(term == "MBSR1") %>% pull(nominalEffSize))`
* `r sprintf("%.03g%%", power %>% filter(term == "MBSR2") %>% pull(power) * 100)` 
  power to detect a standardized effect size of 
  `r sprintf("%.03g", power %>% filter(term == "MBSR2") %>% pull(nominalEffSize))`
